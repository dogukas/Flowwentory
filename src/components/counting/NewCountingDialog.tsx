...
export function NewCountingDialog({ trigger }: NewCountingDialogProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const newEvent = {
            ...
            total_items_planned: stockData.length, // Fix #2: Set from actual stock count
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="grid gap-4 py-4">
                        ...
                        {stockData.length === 0 && (
                            <div className="flex items-start gap-3 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl text-sm">
                                ...
                                <div>
                                    <p className="font-medium text-amber-800 dark:text-amber-300">Stok verisi bulunamadı</p>
                                    <p className="text-amber-600 dark:text-amber-400 text-xs mt-0.5">
                                        Sayım başlatabilmek için önce <strong>Stok Sorgula</strong> sayfasından Excel yükleyin veya Supabase'den veri çekin.
                                    </p>
                                </div>
                            </div>
                        )}
                        ...
                        {stockData.length > 0 && (
                            <div className="flex items-center gap-2 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl text-sm">
                                ...
                                <p className="text-blue-700 dark:text-blue-300">
                                    <strong>{stockData.length.toLocaleString('tr-TR')}</strong> ürün sayıma dahil edilecek.
                                </p>
                            </div>
                        )}
                    ...
                    <DialogFooter>
                        ...
                        <Button
                            ...
                            title={stockData.length === 0 ? "Stok verisi yükleyin" : undefined}
                        >
                            Sayım Oluştur
                        </Button>
                    </DialogFooter>
